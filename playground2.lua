...
function scene:create( event )
    ...
    -- background
    local background = display.newImageRect( "cfDbDHi.png", 320, 580 )
    ...
    bird = display.newImageRect( "birb2.png", 100, 100 )
    bird.x = 16
...
function wall()
    ...
    for i = 0, 7, 1
    do
        local stone = display.newImageRect( "file_download.jfif", 50 , 50)
        stone.x = 295
        stone.y = 70 * i
        stone.myName = "stone"
        ...
        transition.to( stone, { y = stone.y, x=-50, time=5000,
            onComplete = function() display.remove( stone ) end
        } )
        physics.addBody( stone.scoreDisplay, "kinematic", { radius=5})
        transition.to( stone.scoreDisplay, { y = stone.scoreDisplay.y, x=0, time=5000,
...
function scene:show( event )
    ...
    elseif ( phase == "did" ) then
        ...
        gameLoopTimer3 = timer.performWithDelay( 4100, wallLoop, 0 )
...
function scene:hide( event )
    ...
    elseif ( phase == "did" ) then
        ...
        physics.pause()
        transition.cancel()
        composer.removeScene("playground2")
